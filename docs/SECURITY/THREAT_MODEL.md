...
| **Denial of Service (DoS)**| Automated scripts flooding question/feedback forms with high request volume. | Server resource exhaustion, email quota depletion, DB bloat. | In-memory sliding-window rate limiting (`lib/rateLimit.ts`) + progressive slowdown + Zod payload bounds on all write endpoints. |
